...
func main() {
	...
	err := get("http://spb.getoutfit.co:8888/categories?limit=-1", &categories)
...
func printCategories(categories []Category) {
	...
	if get("http://spb.getoutfit.co:8888/offers?count=true&limit=-1", &counter) != nil { return }
...
func printCategory(index int, category Category) int {
	...
	counter := Count{}
	url := fmt.Sprintf("http://spb.getoutfit.co:8888/offers?categoryId=%d&count=true&limit=-1", category.ID)
